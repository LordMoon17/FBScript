...
local BOSS_SPAWN_CFRAME = CFrame.new(572.440918, 154.641174, -1329.18274)
local SIDE_OFFSET = 8
...
local function runGear5Bug()
    if getgenv().CakeQueenGear5Ready then
        loadScript("https://raw.githubusercontent.com/LordMoon17/FBScript/main/Farm/WholeCake/CakeQueen/follow.lua")
        return
    end

    task.wait(0.3)
    loadScript("https://raw.githubusercontent.com/LordMoon17/FBScript/main/Farm/WholeCake/CakeQueen/activate_gear5_bug.lua")
end
...
if rootPart then
    ...
    print("TP al lado de Cake Queen completado: " .. tostring(boss:GetFullName()))
    runGear5Bug()
    return
end
...
print("TP al lado del spawn de Cake Queen completado")
runGear5Bug()
