...
local function disconnectWatcher()
    if getgenv().CakeQueenWatcherConnection then
        getgenv().CakeQueenWatcherConnection:Disconnect()
        getgenv().CakeQueenWatcherConnection = nil
    end
end
...
local function getBossModel(instance)
    ...
    if not model or not model:IsA("Model") or model.Name ~= "Cake Queen" then
        return nil
    end
...
local function onBossDetected(model, source)
    ...
    disconnectWatcher()
    print("Cake Queen detectada por " .. tostring(source) .. ": " .. model:GetFullName())
...
disconnectWatcher()
getgenv().CakeQueenWatcherConnection = Workspace.DescendantAdded:Connect(function(descendant)
    local bossModel = getBossModel(descendant)
    if bossModel then
